...
## Approach
The problem can be solved using dynamic programming, where we build up a table of minimum coin counts for each amount from 0 to the target amount. We iterate over each coin and update the minimum count for each amount. The final result is stored in the last index of the table.
...
- The table `dp` is used to store the minimum coin count for each amount, and is updated iteratively based on the available coins.
- The final result is stored in the last index of the table, and is returned as the minimum coin count for the target amount.