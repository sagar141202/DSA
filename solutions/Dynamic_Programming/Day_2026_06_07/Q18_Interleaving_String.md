...
## Approach
The problem can be solved using dynamic programming by creating a 2D table to store the interleaving status of substrings of `s1` and `s2`. We fill the table in a bottom-up manner, checking if the current characters in `s1` and `s2` match the current character in `s3`.
...
- The base cases are initialized to handle the edge cases where one of the strings is empty.