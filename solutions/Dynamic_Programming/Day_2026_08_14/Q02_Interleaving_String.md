...
## Approach
The problem can be solved using dynamic programming by creating a 2D table to store the interleaving status of substrings of `s1` and `s2`. We can then fill up the table in a bottom-up manner. The interleaving status of `s1` and `s2` can be determined from the table.
...
public:
    bool isInterleave(string s1, string s2, string s3) {
        ...
        // Create a 2D table to store the interleaving status of substrings of s1 and s2
