...
## Problem Statement
The edit distance between two strings is the minimum number of operations (insertions, deletions, and substitutions) required to change one string into the other. Given two strings `word1` and `word2`, find the edit distance between them. The strings only contain lowercase English letters. The length of `word1` and `word2` is at most 500. For example, the edit distance between "kitten" and "sitting" is 3 (replace 'k' with 's', replace 'e' with 'i', append 'g').

## Approach
We will use dynamic programming to solve this problem by creating a 2D array `dp` where `dp[i][j]` represents the edit distance between the first `i` characters of `word1` and the first `j` characters of `word2`. We will fill up the `dp` array in a bottom-up manner.
...
- The edit distance can be calculated by considering the minimum cost of inserting, deleting, or substituting a character.