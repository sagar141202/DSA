...
## Approach
We can solve this problem using a hash table to store the elements we have seen so far and their indices. We iterate through the array and for each element, we check if its complement (target - current element) is in the hash table. If it is, we return the indices of the current element and its complement.
...
## Key Takeaways
- Use a hash table to store the elements and their indices for efficient lookups.
- Iterate through the array and check if the complement of each element is in the hash table.
