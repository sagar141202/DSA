...
## Approach
We will use a hash map to store the elements we have seen so far and their indices. We iterate over the array and for each element, we check if its complement (target - current element) is in the hash map. If it is, we return the indices of the current element and its complement.
...
- Iterate over the array only once to reduce the time complexity.
