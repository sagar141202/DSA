...
## Approach
We will use a hash map to store the numbers we have seen so far and their indices. Then, for each number, we check if its complement (target - number) is in the hash map. If it is, we return the indices of the current number and its complement.
...
public:
    vector<int> twoSum(vector<int>& nums, int target) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            ...
            // If the complement is not in the hash map, add the current number to the hash map
            numToIndex[nums[i]] = i;
        }
...
- Iterate over the array and check if the complement of each number is in the hash map.
