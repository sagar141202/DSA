...
## Approach
We will use a hash table to store the numbers we have seen so far and their indices. Then, for each number, we check if its complement (target - current number) is in the hash table. If it is, we return the indices of the current number and its complement.
...
- Iterate over the array only once to achieve O(n) time complexity.
- Handle edge cases, such as an empty input array or no solution found.