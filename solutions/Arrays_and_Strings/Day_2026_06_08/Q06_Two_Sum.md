...
## Approach
We can solve this problem using a hash table to store the numbers we have seen so far and their indices. We iterate through the array, and for each number, we check if its complement (target - current number) is in the hash table. If it is, we return the indices of the current number and its complement.
...
- Iterate through the array and check if the complement of each number is in the hash table.
