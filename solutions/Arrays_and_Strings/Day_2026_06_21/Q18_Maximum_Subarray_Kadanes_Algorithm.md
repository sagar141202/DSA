...
## Approach
Kadane's algorithm is used to solve this problem, which involves iterating through the array and at each step, deciding whether to continue the current subarray or start a new one. The algorithm keeps track of the maximum sum of subarray ending at each position.
...
- The key to the algorithm is to keep track of the maximum sum of subarray ending at each position and update it accordingly.