...
## Approach
We will use a hash map to store the elements of the array as we iterate through it, keeping track of the index of each element. This allows us to check if the complement of the current element (i.e., the value needed to reach the target) is already in the hash map. If it is, we return the indices of the current element and its complement.
...
- Check for the complement of each element to find the solution.