...
## Approach
We can use a hash table to store the numbers we have seen so far and their indices. Then, for each number, we check if its complement (target - number) is in the hash table. If it is, we return the indices of the current number and its complement. This approach allows us to find the solution in a single pass through the array.
...
- Iterate through the array only once to find the solution.
- Check if the complement of the current number is in the hash table to find the solution.