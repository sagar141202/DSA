...
## Approach
We will use a hash table to store the elements of the array as we iterate through it, along with their indices. This allows us to check if the difference between the target and the current element exists in the hash table in constant time. If it does, we return the indices of these two elements.
