...
## Approach
We will use a hash table to store the numbers we have seen so far and their indices. Then we iterate through the array and for each number, we check if its complement (i.e., the number that we need to add to it to get the target) is in the hash table. If it is, we return the indices of the current number and its complement.
...
- Iterate through the array and for each number, check if its complement is in the hash table to find the solution.
- If the complement is found, return the indices of the current number and its complement. Otherwise, add the current number and its index to the hash table.