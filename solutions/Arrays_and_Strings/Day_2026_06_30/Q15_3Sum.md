...
## Problem Statement
Given an integer array `nums`, find all unique triplets in the array which gives the sum of zero. The solution should not contain duplicate triplets. For example, given `nums = [-1,0,1,2,-1,-4]`, the output should be `[[-1,-1,2],[-1,0,1]]`. The array can contain duplicate elements and the length of the array is in the range `[0, 3000]`. The elements of the array are in the range `[-10^5, 10^5]`.

## Approach
The approach to solve this problem is to use a two-pointer technique with sorting. First, sort the array and then fix one element, and use two pointers to find the other two elements that sum up to the negation of the fixed element.
