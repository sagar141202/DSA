...
## Problem Statement
Given an integer array `nums`, find all unique triplets in the array that sum to zero. The solution should not contain duplicate triplets. The input array can contain duplicate elements, and the length of the array is between 3 and 3000. For example, given the array `[-1,0,1,2,-1,-4]`, the output should be `[[-1,-1,2],[-1,0,1]]`.

## Approach
The algorithm uses a two-pointer technique to find the triplets. First, it sorts the array and then iterates over each element, using two pointers to find a pair of elements that sum to the negation of the current element. The algorithm skips duplicate elements to avoid duplicate triplets.
