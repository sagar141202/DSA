...
## Approach
We can use a hash map to store the numbers we have seen so far and their indices. Then, for each number, we check if its complement (target - number) is in the hash map. If it is, we return the indices of the current number and its complement.
...
- Check if the complement of the current number is in the hash map to find the solution.