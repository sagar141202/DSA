...
## Problem Statement
Given an array of integers `nums` and an integer `k`, return the maximum value in each subarray of size `k`. The subarray can be formed by sliding a window of size `k` over the array. For example, given `nums = [1, 3, -1, -3, 5, 3, 6, 7]` and `k = 3`, the output should be `[3, 3, 5, 5, 6, 7]`. The constraints are `1 <= nums.length <= 10^5` and `1 <= k <= nums.length`.
