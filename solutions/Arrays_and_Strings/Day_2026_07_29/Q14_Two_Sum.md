...
## Approach
We can use a hash table to store the elements we have seen so far and their indices. Then, for each element, we check if its complement (target - current element) is in the hash table. If it is, we return the indices of the current element and its complement.
...
## Key Takeaways
- Use a hash table to store elements and their indices for efficient lookup.
- Iterate over the array only once to minimize time complexity.
- Check for the complement of each element in the hash table to find the solution.