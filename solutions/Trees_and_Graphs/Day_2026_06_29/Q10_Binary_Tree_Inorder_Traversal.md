...
Given the root of a binary tree, return the inorder traversal of its nodes' values. Inorder traversal visits the left subtree, the current node, and then the right subtree. For example, given the following binary tree: 
   ...
   / \
  1   3
The inorder traversal is [1, 2, 3, 4, 5]. The binary tree node has the following structure: 
...
- The recursive approach is often simpler to implement, but the iterative approach can be more efficient in terms of memory usage.
