...
public:
    bool hasPathSum(TreeNode* root, int targetSum) {
        ...
        // Recursive case: if the current node is not a leaf, recursively check its children
        targetSum -= root->val;
...
- Base case: if the tree is empty, return false.
- Recursive case: if the current node is a leaf and its value equals the target sum, return true; otherwise, recursively check its children.