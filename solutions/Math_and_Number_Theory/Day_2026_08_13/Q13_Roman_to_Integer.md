...
## Problem Statement
Roman numerals are represented by seven different symbols: I, V, X, L, C, D, and M, which represent the numbers 1, 5, 10, 50, 100, 500, and 1000, respectively. The problem requires converting a Roman numeral to an integer. For example, III is 3, IV is 4, IX is 9, LVIII is 58, and MCMXCIV is 1994. The input will be a string containing only the characters I, V, X, L, C, D, and M, and the output should be the integer equivalent.

## Approach
The algorithm works by iterating through the Roman numeral string and adding the value of each symbol to a running total. If the current symbol is less than the next symbol, it means that the current symbol should actually be subtracted from the total (because in Roman numerals, a smaller number in front of a larger number means subtraction).
...
- Iterate through the string and add or subtract the value of each symbol based on whether it is less than the next symbol.
