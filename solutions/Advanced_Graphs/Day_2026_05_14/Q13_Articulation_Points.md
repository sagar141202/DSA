...
## Problem Statement
Given an undirected graph, find all the articulation points in the graph. An articulation point is a node that, when removed, increases the number of connected components in the graph. The graph is represented as an adjacency list. The function should return a list of all articulation points in the graph.

## Approach
The approach is to use Depth-First Search (DFS) to traverse the graph and keep track of the discovery time and low value of each node. A node is an articulation point if it is the root of the DFS tree and has at least two children, or if it is not the root and the low value of one of its children is greater than or equal to its discovery time.
...
void dfs(int node, int parent, vector<vector<int>>& graph, vector<bool>& visited, 
         vector<int>& disc, vector<int>& low, vector<bool>& articulation, int& time) {
    visited[node] = true;
...
- The DFS approach is used to find articulation points in an undirected graph.
