...
## Approach
The Bellman-Ford algorithm works by relaxing all edges V-1 times, where V is the number of vertices. This is because the shortest path from the source vertex to any other vertex can have at most V-1 edges. The algorithm then checks for negative weight cycles by trying to relax all edges one more time.
...
Shortest distance from source to vertex 1: -1
Shortest distance from source to vertex 2: 2
Shortest distance from source to vertex 3: -2
Shortest distance from source to vertex 4: 1
```
