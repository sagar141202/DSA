...
## Approach
We will use a Trie data structure to store the dictionary words and then perform a depth-first search on the board to find all words. The Trie will allow us to efficiently check if a word is in the dictionary, and the depth-first search will allow us to explore all possible paths on the board.
...
- Use a visited matrix to avoid revisiting the same cell.