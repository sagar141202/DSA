...
## Approach
The problem can be solved using recursion and backtracking, where we try adding each number to the current combination and recursively call the function with the remaining target. We use a helper function to explore all possible combinations. The base case is when the target becomes zero, at which point we add the current combination to the result.
...
## Key Takeaways
- Use recursion and backtracking to explore all possible combinations of candidates that sum to the target.
- Use a helper function to simplify the code and improve readability.
- Be careful with the base cases to avoid infinite recursion or incorrect results.