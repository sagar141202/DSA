...
## Approach
The problem can be solved using recursion and backtracking. Start with an empty combination and try to add each number from the candidates. If the sum exceeds the target, stop exploring the current combination. If the sum equals the target, add the combination to the result.
...
- Start with an empty combination and try to add each number from the candidates.
