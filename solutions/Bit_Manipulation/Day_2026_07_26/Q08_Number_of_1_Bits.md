...
## Approach
The approach to solve this problem is to use bit manipulation to count the number of 1 bits in the binary representation of the input integer. We can use a while loop to iterate through each bit in the integer and use the bitwise AND operator to check if the current bit is 1. If it is, we increment the count.
...
- The time complexity of this solution is O(log n) because we are iterating through each bit in the binary representation of the input integer.