...
## Problem Statement
Given an integer `n`, write a function to count the number of bits that are set (i.e., 1) in the binary representation of all numbers from 0 to `n`. For example, if `n` is 5, the binary representations are 0 (0), 1 (1), 10 (2), 11 (3), 100 (4), and 101 (5). The total number of set bits is 0 + 1 + 1 + 2 + 1 + 2 = 7.
...
- We can also implement our own function to count the set bits by using a while loop and bitwise operations.