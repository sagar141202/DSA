...
## Problem Statement
Given an array of integers containing only 0s, 1s, and 2s, sort the array in a single pass such that all 0s come first, followed by all 1s, and then all 2s. This is also known as the Dutch Flag problem. The array should be sorted in-place, meaning no additional space should be used.
...
- The use of swap operations allows the algorithm to modify the input array in-place, without using any additional space.